...
def main(trial):
    ...
    args.alpha = trial.suggest_float('alpha', 0.0, 12.0)
    args.lambdas[0] = trial.suggest_float('retrieval_lambda', 0.0, 12.0)
    args.lambdas[1] = trial.suggest_float('abstract_lambda', 0.0, 12.0)
    args.lambdas[2] = trial.suggest_float('rationale_lambda', 0.0, 12.0)
    printf(args)
    k_train = 12
    tokenizer = AutoTokenizer.from_pretrained(args.model)
    ...
    retrieval_score = evaluation_abstract_retrieval(model, test_set, args, tokenizer)
    return retrieval_score['abstract_recall']
    # return (res['sentence_selection']['f1'] + res['sentence_label']['f1'] + res['abstract_label_only']['f1'] + res['abstract_rationalized']['f1']) / 4


if __name__ == "__main__":
    # print(80)
    study_name = 'scifact-study-retrieval-BioBert'  # Unique identifier of the study.
    # study = opt.create_study(study_name=study_name, directions=['maximize', 'maximize', 'maximize', 'maximize'], storage='sqlite:///scifact.db')
    # study = opt.load_study(study_name, storage='sqlite:///scifact.db')
    study = opt.create_study(study_name=study_name, direction='maximize', storage='sqlite:///scifact.db', load_if_exists=True)
    study.optimize(main, n_trials=50)
    joblib.dump(study, "study-retrieval-BioBert.pkl")
