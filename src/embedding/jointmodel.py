...
class AbstractAttention(nn.Module):
    """
    Hierarchical attention. abstract representation for abstract retrieval.
    """
    ...
    def forward(self, x, token_mask, claim_reps):
        ...
        # sentence_att_scores = sentence_att_scores[:, range(0, sentence_att_scores.shape[1])]
        sentence_att_scores = torch.softmax(sentence_att_scores, dim=-1)
        # print('abstract_sentence_reps: ', sentence_reps)
        return output, sentence_att_scores, word_att_scores
...
class JointModelClassifier(nn.Module):
    ...
    def forward(self, encoded_dict, transformation_indices, abstract_label=None, rationale_label=None,
                retrieval_label=None, train=False, retrieval_only=False, rationale_sample=1, retrieval_state=False):
        ...
        # sentence_token = range(1, batch_indices.shape[1])  # pretraining. FEVER Dataset without title.
        ...
        abstract_retrieval, sentence_att_scores, word_att_scores = self.abstract_retrieval(title_abstract_tokens,
        ...
        # print('rationale_pred:', rationale_pred)
        # print('abstract_sentence_scores:', sentence_att_scores)
        # print('abstract_word_att_scores:', word_att_scores)
        # print(100 * '*')
